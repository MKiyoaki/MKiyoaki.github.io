...
document.addEventListener('DOMContentLoaded', () => {
  const waveSections = document.querySelectorAll('.has-waves-background');
...
function initWavesForSection(sectionElement) {
  // --- 核心改动：自动检测深色模式 ---
  const isDarkMode = sectionElement.classList.contains('dark');

  // 根据模式，从您的主题变量中选择颜色
  const config = {
    // 浅色模式: primary-300 作背景, primary-600 作线条
    // 深色模式: primary-600 作背景, primary-300 作线条
    backgroundColor: isDarkMode
      ? `rgb(var(--color-primary-600))`
      : `rgb(var(--color-primary-300))`,
    lineColor: isDarkMode
      ? `rgba(var(--color-primary-300), 0.4)` // 深色背景配浅色线条(带透明度)
      : `rgba(var(--color-primary-600), 0.4)`, // 浅色背景配深色线条(带透明度)
  ...
  // 将JS中计算好的背景色应用到CSS变量上
  container.style.setProperty('--waves-bg-color', config.backgroundColor);
