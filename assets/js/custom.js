...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const config = {
    lineColor: "rgba(0, 0, 0, 0.2)",  // 线的颜色 (建议带一点透明度)
    backgroundColor: "transparent",     // 背景颜色, "transparent" 表示透明
    waveSpeedX: 0.01,               // X轴波浪速度
    waveSpeedY: 0.005,                // Y轴波浪速度
    waveAmpX: 16,                     // X轴波浪振幅
    waveAmpY: 8,                     // Y轴波浪振幅
    xGap: 10,                         // 垂直线的间距
    yGap: 32,                         // 水平线的间距
    friction: 0.925,                  // 鼠标交互后的摩擦力 (值越小，恢复越快)
    tension: 0.0025,                   // 鼠标交互后的张力
    maxCursorMove: 50,               // 鼠标交互影响的最大范围
  };
  ...
  if (!container) {
    console.log('Waves background container not found.');
    return; // 如果没找到容器，则不执行任何操作
  }
  ...
  function setLines() {
    ...
    for (let i = 0; i <= totalLines; i++) {
      ...
      for (let j = 0; j <= totalPoints; j++) {
        pts.push({
          x: xStart + config.xGap * i, y: yStart + config.yGap * j,
